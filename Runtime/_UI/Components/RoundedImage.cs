...
public class RoundedImage : Image
{
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        //0，1
        vh.AddVert(new Vector3(v.x, v.w - (topLeft?radius:0)), color32, new Vector2(uv.x, uv.w - (topLeft ? uvRadiusY : 0)));
        vh.AddVert(new Vector3(v.x, v.y + (bottomLeft ? radius : 0)), color32, new Vector2(uv.x, uv.y + (bottomLeft ? uvRadiusY : 0)));

        //2，3，4，5
        vh.AddVert(new Vector3(v.x + (topLeft ? radius : 0), v.w), color32, new Vector2(uv.x + (topLeft ? uvRadiusX : 0), uv.w));
        ...
        vh.AddVert(new Vector3(v.x + (bottomLeft ? radius : 0), v.y), color32, new Vector2(uv.x + (bottomLeft ? uvRadiusX : 0), uv.y));

        //6，7，8，9
        vh.AddVert(new Vector3(v.z - (topRight ? radius : 0), v.w), color32, new Vector2(uv.z - (topRight ? (topRight ? uvRadiusX : 0)  : 0) , uv.w));
        ...
        vh.AddVert(new Vector3(v.z - (bottomRight ? radius : 0), v.y), color32, new Vector2(uv.z - (bottomRight ? uvRadiusX : 0), uv.y));

        //10，11
        vh.AddVert(new Vector3(v.z, v.w - (topRight ? radius : 0)), color32, new Vector2(uv.z, uv.w - (topRight ? uvRadiusY : 0)));
        vh.AddVert(new Vector3(v.z, v.y + (bottomRight ? radius : 0)), color32, new Vector2(uv.z, uv.y + (bottomRight ? uvRadiusY : 0) ));
